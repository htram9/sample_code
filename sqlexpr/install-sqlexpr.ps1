...
Get-ChildItem
Set-Location -Path kit


# Install SQL Server Express 2022 Silently
.\setup.exe /q /ACTION=Install /FEATURES=SQLEngine,Replication /INSTANCENAME=$InstanceName /SQLSVCACCOUNT="NT AUTHORITY\SYSTEM"  /SQLSYSADMINACCOUNTS="BUILTIN\ADMINISTRATORS" /AGTSVCACCOUNT="NT AUTHORITY\Network Service" /TCPENABLED=1 /IACCEPTSQLSERVERLICENSETERMS /SQLUSERDBDIR='C:\Mounted_Data\MSSQLSERVER\SQLUSERDBDIR' /SQLUSERDBLOGDIR='C:\Mounted_Data\MSSQLSERVER\SQLUSERDBDIR'
